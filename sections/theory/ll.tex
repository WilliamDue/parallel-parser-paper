...
The first and follow-set algorithms described takes heavy inspiration from Mogensens book Introduction to Compiler Design \cite[p. 55-65]{Mogensen} and the parser notes \cite[p. 10-15]{notes:parser} by Sestoft and Larsen. The modifications are mainly using the $LL(k)$ extension described in the Wikipedia article in the section ``Constructing an LL(k) parsing table''\footnote{At the time of writing the Wikipedia article does have a description of constructing first and follow-sets for $k > 1$. The problem is the algorithm described does not fullfill the definition of first and follow-sets that is being used in the LLP paper \cite[p. 5]{Vagner2007}.} \cite{wiki:LL_parser}.

For the construction of a $\LLP(q,k)$ parser generator the construction of first and follow-set \cite[5]{Vagner2007} and a $\LL(k)$ parser generator is needed.

\subsubsection{\texorpdfstring{$\FIRST_k$}{TEXT} and \texorpdfstring{$\FOLLOW_k$}{TEXT}}
In this section a short explanation of the construction of first and follow sets will be given. During the research of this project it was found $k = 1$ was quite often explained but never $k > 1$ since this was often seen as trivial.
...
\begin{definition}[Truncated product]
    Let $G = (N, T, P, S)$ be a context-free grammar, $A, B \subseteq (N \cup T)^*$\footnote{Let $Z$ be an alphabet, $Z^*$ is defined to be $\epsilon \in Z^*$ and $Z^* \defeq \{tv : t \in Z, v \in Z^*\}$} be sets of symbol strings and $\omega, \delta \in (T \cup N)^*$. The truncated product is defined in the following way.
    \begin{gather*}
...
\begin{algorithm}[Solving $\FIRST_k$ sets]
    ...
    Let $G = (N, T, P, S)$ be a context-free grammar and $\FIRST_k: (N \cup T)^* \to \mathbb{P}(T^*)$\footnote{$\mathbb{P}$ is the powerset.}. The first-sets for a given string can be solved as followed.
    \begin{align*}
...
\begin{enumerate}
    ...
    \item Let $\mathcal{M}_f$ be the dictionary such that $f$ is the greatest index. It then holds that $\FIRST_k(\omega) = \FIRST'_k(\omega, \mathcal{M}_f)$ if $k$ stays fixed.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Let $\mathcal{M}_f$ be the dictionary such that $f$ is the greatest index. Let $\mathcal{M}_u$ be a dictionary where $\mathcal{M}_u(A) = \{\alpha : \alpha \square^* \in \mathcal{M}_f(A)\}$ for all $A \in N$.
    \item It then holds that $\FOLLOW_k(A) = \mathcal{M}_u(A)$ if $k$ stays fixed for grammar $G$.
\end{enumerate}
Using $\FIRST_k$ and $\FOLLOW_k$ a $\LL(k)$ table which maps nonterminals and $k$ or less terminals to a production index.
\begin{definition}[$\LL(k)$ table]
    Let $G = (N, T, P, S)$ be a $\LL(k)$ context-free grammar and $\tau : N \times T^* \to \mathbb{P}(\mathbb{N})$. For a given production $A \to \delta = p_i \in P$ where $i \in \{0, ..., |P| - 1\}$ is a unique index.
    \begin{gather*}
...
\begin{definition}[LL parser configuration]
    ...
    \end{align*}
    For a $\LL(k)$ parser configuration $(w, \alpha, \pi)$ where $w$ denote the input string, $\alpha$ denote the pushdown store and $\pi$ denote the sequence of rules used to derive the consumed input string.
\end{definition}
\noindent When using deterministic $\LL(k)$ parsing you want to create a parsing function. To define how such a function is created the LL parsing relation is defined. This relation holds if the left LL configuration can turn into the right LL configuration after parsing the left configuration.
\begin{gather*}
...
\end{gather*}
The pop condition means if the left configuration has an input string and a pushdown store with the same first terminal. And the right configuration matches the left configuration, but the first terminal is popped.

The deriving condition means if the pushdown store $\alpha$ of the left LL configuration has a first element that is a nonterminal $A$. If the nonterminal together with the lookahead string results in a valid production index. And the right-hand side of the production is on top of the pushdown store on the right.
